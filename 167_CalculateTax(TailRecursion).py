...
class Solution:
    ...
    def calcTax(self, index, salary, limit, currTax):
        ...
        if taxable == None or salary < taxable:
            return currTax + (salary * perc)

        #recursive case
        currTax = currTax + taxable * perc


        return self.calcTax(index+1, salary - taxable, self.levels[index][0], currTax)

sol = Solution()

print(sol.calcTax(0, 25000, 0, 0))
